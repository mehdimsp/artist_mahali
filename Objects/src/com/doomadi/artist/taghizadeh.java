...
public class taghizadeh extends Activity implements B4AActivity{
	public static taghizadeh mostCurrent;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (processBA == null) {
			processBA = new BA(this.getApplicationContext(), null, null, "com.doomadi.artist", "com.doomadi.artist.taghizadeh");
			...
			if (p != null && p != this) {
                BA.LogInfo("Killing previous instance (taghizadeh).");
				p.finish();
			}
		}
	...
	private void afterFirstLayout() {
        if (this != mostCurrent)
			return;
		activityBA = new BA(this, layout, processBA, "com.doomadi.artist", "com.doomadi.artist.taghizadeh");
        ...
        if (BA.isShellModeRuntimeCheck(processBA)) {
			...
			processBA.raiseEvent2(null, true, "CREATE", true, "com.doomadi.artist.taghizadeh", processBA, activityBA, _activity, anywheresoftware.b4a.keywords.Common.Density, mostCurrent);
			_activity.reinitializeForShell(activityBA, "activity");
		}
        initializeProcessGlobals();		
        initializeGlobals();
        
        BA.LogInfo("** Activity (taghizadeh) Create, isFirst = " + isFirst + " **");
        processBA.raiseEvent2(null, true, "activity_create", false, isFirst);
		isFirst = false;
		if (this != mostCurrent)
			return;
        processBA.setActivityPaused(false);
        BA.LogInfo("** Activity (taghizadeh) Resume **");
...
@Override
 ...
 protected void onStop() {
	...
	public void onPause() {
        ...
        BA.LogInfo("** Activity (taghizadeh) Pause, UserClosed = " + activityBA.activity.isFinishing() + " **");
        if (mostCurrent != null)
...
mostCurrent._http._initialize(processBA,"http",taghizadeh.getObject());
 //BA.debugLineNum = 102;BA.debugLine="LoadDB(\"SELECT * FROM taghizadeh ORDER BY id DESC";
_loaddb("SELECT * FROM taghizadeh ORDER BY id DESC LIMIT 0,10000000000000");
...
if (_keycode==anywheresoftware.b4a.keywords.Common.KeyCodes.KEYCODE_BACK) { 
 //BA.debugLineNum = 611;BA.debugLine="StartActivity(mashhad)";
...
mostCurrent._http._initialize(processBA,"job3",taghizadeh.getObject());
...
_edit._initialize(processBA,"edit",taghizadeh.getObject());
 //BA.debugLineNum = 374;BA.debugLine="edit.PostString(\"http://doholi.com/edit/edit32.ph";
_edit._poststring("http://doholi.com/edit/edit32.php","VEIW="+BA.NumberToString(_asa)+"&id="+mostCurrent._qqqq);
