...
public class delnavaz extends Activity implements B4AActivity{
	public static delnavaz mostCurrent;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (processBA == null) {
			processBA = new BA(this.getApplicationContext(), null, null, "com.doomadi.artist", "com.doomadi.artist.delnavaz");
			...
			if (p != null && p != this) {
                BA.LogInfo("Killing previous instance (delnavaz).");
				p.finish();
			}
		}
	...
	private void afterFirstLayout() {
        if (this != mostCurrent)
			return;
		activityBA = new BA(this, layout, processBA, "com.doomadi.artist", "com.doomadi.artist.delnavaz");
        ...
        if (BA.isShellModeRuntimeCheck(processBA)) {
			...
			processBA.raiseEvent2(null, true, "CREATE", true, "com.doomadi.artist.delnavaz", processBA, activityBA, _activity, anywheresoftware.b4a.keywords.Common.Density, mostCurrent);
			_activity.reinitializeForShell(activityBA, "activity");
		}
        initializeProcessGlobals();		
        initializeGlobals();
        
        BA.LogInfo("** Activity (delnavaz) Create, isFirst = " + isFirst + " **");
        processBA.raiseEvent2(null, true, "activity_create", false, isFirst);
		isFirst = false;
		if (this != mostCurrent)
			return;
        processBA.setActivityPaused(false);
        BA.LogInfo("** Activity (delnavaz) Resume **");
...
@Override
 ...
 protected void onStop() {
	...
	public void onPause() {
        ...
        BA.LogInfo("** Activity (delnavaz) Pause, UserClosed = " + activityBA.activity.isFinishing() + " **");
        if (mostCurrent != null)
...
mostCurrent._http._initialize(processBA,"http",delnavaz.getObject());
 //BA.debugLineNum = 102;BA.debugLine="LoadDB(\"SELECT * FROM delnavaz ORDER BY id DESC L";
_loaddb("SELECT * FROM delnavaz ORDER BY id DESC LIMIT 0,10000000000000");
...
if (_keycode==anywheresoftware.b4a.keywords.Common.KeyCodes.KEYCODE_BACK) { 
 //BA.debugLineNum = 611;BA.debugLine="StartActivity(torbatjam)";
...
mostCurrent._http._initialize(processBA,"job3",delnavaz.getObject());
...
_edit._initialize(processBA,"edit",delnavaz.getObject());
 //BA.debugLineNum = 374;BA.debugLine="edit.PostString(\"http://doholi.com/edit/edit35.ph";
_edit._poststring("http://doholi.com/edit/edit35.php","VEIW="+BA.NumberToString(_asa)+"&id="+mostCurrent._qqqq);
