...
public class baghshani extends Activity implements B4AActivity{
	public static baghshani mostCurrent;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (processBA == null) {
			processBA = new BA(this.getApplicationContext(), null, null, "com.doomadi.artist", "com.doomadi.artist.baghshani");
			...
			if (p != null && p != this) {
                BA.LogInfo("Killing previous instance (baghshani).");
				p.finish();
			}
		}
	...
	private void afterFirstLayout() {
        if (this != mostCurrent)
			return;
		activityBA = new BA(this, layout, processBA, "com.doomadi.artist", "com.doomadi.artist.baghshani");
        ...
        if (BA.isShellModeRuntimeCheck(processBA)) {
			...
			processBA.raiseEvent2(null, true, "CREATE", true, "com.doomadi.artist.baghshani", processBA, activityBA, _activity, anywheresoftware.b4a.keywords.Common.Density, mostCurrent);
			_activity.reinitializeForShell(activityBA, "activity");
		}
        initializeProcessGlobals();		
        initializeGlobals();
        
        BA.LogInfo("** Activity (baghshani) Create, isFirst = " + isFirst + " **");
        processBA.raiseEvent2(null, true, "activity_create", false, isFirst);
		isFirst = false;
		if (this != mostCurrent)
			return;
        processBA.setActivityPaused(false);
        BA.LogInfo("** Activity (baghshani) Resume **");
...
@Override
 ...
 protected void onStop() {
	...
	public void onPause() {
        ...
        BA.LogInfo("** Activity (baghshani) Pause, UserClosed = " + activityBA.activity.isFinishing() + " **");
        if (mostCurrent != null)
...
mostCurrent._http._initialize(processBA,"http",baghshani.getObject());
 //BA.debugLineNum = 102;BA.debugLine="LoadDB(\"SELECT * FROM baghshani ORDER BY id DESC";
_loaddb("SELECT * FROM baghshani ORDER BY id DESC LIMIT 0,10000000000000");
...
mostCurrent._http._initialize(processBA,"job3",baghshani.getObject());
...
_edit._initialize(processBA,"edit",baghshani.getObject());
