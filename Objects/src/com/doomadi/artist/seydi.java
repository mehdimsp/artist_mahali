...
public class seydi extends Activity implements B4AActivity{
	public static seydi mostCurrent;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (processBA == null) {
			processBA = new BA(this.getApplicationContext(), null, null, "com.doomadi.artist", "com.doomadi.artist.seydi");
			...
			if (p != null && p != this) {
                BA.LogInfo("Killing previous instance (seydi).");
				p.finish();
			}
		}
	...
	private void afterFirstLayout() {
        if (this != mostCurrent)
			return;
		activityBA = new BA(this, layout, processBA, "com.doomadi.artist", "com.doomadi.artist.seydi");
        ...
        if (BA.isShellModeRuntimeCheck(processBA)) {
			...
			processBA.raiseEvent2(null, true, "CREATE", true, "com.doomadi.artist.seydi", processBA, activityBA, _activity, anywheresoftware.b4a.keywords.Common.Density, mostCurrent);
			_activity.reinitializeForShell(activityBA, "activity");
		}
        initializeProcessGlobals();		
        initializeGlobals();
        
        BA.LogInfo("** Activity (seydi) Create, isFirst = " + isFirst + " **");
        processBA.raiseEvent2(null, true, "activity_create", false, isFirst);
		isFirst = false;
		if (this != mostCurrent)
			return;
        processBA.setActivityPaused(false);
        BA.LogInfo("** Activity (seydi) Resume **");
...
@Override
 ...
 protected void onStop() {
	...
	public void onPause() {
        ...
        BA.LogInfo("** Activity (seydi) Pause, UserClosed = " + activityBA.activity.isFinishing() + " **");
        if (mostCurrent != null)
...
mostCurrent._http._initialize(processBA,"http",seydi.getObject());
 //BA.debugLineNum = 102;BA.debugLine="LoadDB(\"SELECT * FROM sadidy ORDER BY id DESC LIM";
_loaddb("SELECT * FROM sadidy ORDER BY id DESC LIMIT 0,10000000000000");
...
mostCurrent._http._initialize(processBA,"job3",seydi.getObject());
...
_edit._initialize(processBA,"edit",seydi.getObject());
 //BA.debugLineNum = 374;BA.debugLine="edit.PostString(\"http://doholi.com/edit/edit13.ph";
_edit._poststring("http://doholi.com/edit/edit13.php","VEIW="+BA.NumberToString(_asa)+"&id="+mostCurrent._qqqq);
